<x-app-layout>
    ...
    <form method="POST" action="{{ route('livros.store') }}" enctype="multipart/form-data" class="space-y-4 rounded-xl border border-cyan-300/20 bg-slate-900/70 p-6">
        @csrf

        <div class="grid gap-4 md:grid-cols-2">
            <div>
                <label class="label"><span class="label-text">ISBN</span></label>
                <input type="text" name="isbn" value="{{ old('isbn') }}" class="input input-bordered w-full" required />
            ...
            <div>
                ...
                <select name="editora_id" class="select select-bordered w-full" required>
                    ...
                    @foreach ($editoras as $editora)
                        <option value="{{ $editora->id }}" @selected((int) old('editora_id') === $editora->id)>{{ $editora->nome }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="number" step="0.01" min="0" name="preco" value="{{ old('preco') }}" class="input input-bordered w-full" required />
        ...
        <div>
            ...
            <select name="autores[]" multiple class="select select-bordered w-full h-40" required>
                @foreach ($autores as $autor)
                    <option value="{{ $autor->id }}" @selected(in_array($autor->id, old('autores', [])))>{{ $autor->nome }}</option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <textarea name="sinopse" class="textarea textarea-bordered w-full" rows="6">{{ old('sinopse') }}</textarea>
        </div>
